...
public class ProfitReportsQueryService : IProfitReportsQueryService
{
    ...
    {
        var rows = await (from o in ScopedOrders().AsNoTracking().Where(x => x.DeliveredAt != null && x.DeliveredAt >= from && x.DeliveredAt <= to)
        ...
        string key(dynamic r) => groupBy switch
        {
        ...
        return rows.GroupBy(key)
            .Select(g =>
            {
                ...
                return new ProfitBreakdownRowDto(
                    groupBy.Contains("governorate") ? first.Governorate : (groupBy == "governorate" ? first.Governorate : "-"),
                    groupBy.Contains("size") ? first.Size : "-",
                    groupBy.Contains("pricing") || groupBy == "pricingCategory" ? first.PricingCategory : "-",
                    g.Count(),
    ...
    {
        return await (from o in ScopedOrders().AsNoTracking().Where(x => x.DeliveredAt != null && x.DeliveredAt >= from && x.DeliveredAt <= to && x.DeliveryAgentId != null)
