...
public class MerchantStatementsQueryService : IMerchantStatementsQueryService
{
    ...
    public async Task<MerchantStatementWithPaymentsDto> GetStatementAsync(Guid merchantId, DateTime from, DateTime to, int page, int size, bool onlyReadyForSettlement, CancellationToken cancellationToken = default)
    {
        ...
        var rowsQuery = onlyReadyForSettlement ? baseQuery.Where(x => x.IsDeliveryAgentSettled) : baseQuery;

        var orderRowsRaw = await (from o in rowsQuery
                                  ...
                                  select new { o.Id, Date = o.DeliveredAt!.Value, o.OrderNumber, o.CustomerName, Gov = gov != null ? gov.Name : "-", Size = o.OrderSize.HasValue ? o.OrderSize.Value.ToString() : "-", o.DeliveredPriceWithDelivery, o.AmountToCollect, o.DeliveryFeeApplied, o.MerchantDueAmount, o.IsDeliveryAgentSettled, o.IsMerchantSettled, o.HasReturn, o.InternalNote })
            .Skip((normalizedPage - 1) * normalizedSize)
            .Take(normalizedSize)
            .ToListAsync(cancellationToken);

        var orders = orderRowsRaw.Select(x =>
        {
            ...
            return new MerchantStatementRowDto(x.Date, x.OrderNumber, x.CustomerName, x.Gov, x.Size ?? "-", delivered, x.DeliveryFeeApplied ?? 0m, x.MerchantDueAmount ?? 0m, x.IsDeliveryAgentSettled, x.IsMerchantSettled, x.HasReturn, x.InternalNote);
        }).ToList();

        var payments = await ScopedPayments(_dbContext.MerchantPayments.AsNoTracking())
            .Where(x => x.MerchantId == merchantId && x.PaymentDate >= from && x.PaymentDate <= to)
            .OrderByDescending(x => x.PaymentDate)
            .Select(x => new MerchantPaymentRowDto(x.PaymentDate, x.Amount, x.Method.ToString(), x.ReferenceNo, x.Notes))
            .ToListAsync(cancellationToken);

        return new MerchantStatementWithPaymentsDto(summary, orders, payments);
    }

    private IQueryable<Domain.Operations.Entities.Order> ScopedOrders()
    {
        if (_requestContext.IsSaasAdmin) return _dbContext.Orders.AsQueryable();
    ...
    private IQueryable<Domain.Accounting.Entities.MerchantPayment> ScopedPayments(IQueryable<Domain.Accounting.Entities.MerchantPayment> query)
    {
